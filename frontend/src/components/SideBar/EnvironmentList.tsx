...
export const EnvironmentList: React.FC<EnvironmentListProps> = ({
  ...
  return (
    <div className="px-2 pb-20 space-y-3">
      <div className="px-1 py-2 flex items-center justify-between text-[10px] text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200 dark:border-white/5 mb-2">
      ...
      {envVariables.map((v, i) => (
        <div
          ...
          className={`
            ...
            ${v.enabled
              ? 'bg-white border-slate-200 hover:border-slate-300 dark:bg-white/[0.02] dark:border-white/10 dark:hover:border-white/20'
        ...
        >
          ...
          <div className="flex flex-col gap-1 p-2 pl-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                <div className="relative">
                  <select
                    className={`appearance-none bg-white dark:bg-near-black border border-slate-200 dark:border-white/10 rounded px-1.5 py-0.5 text-[9px] font-bold uppercase outline-none cursor-pointer hover:border-slate-300 dark:hover:border-white/20 transition-colors ${
                ...
                <button
                  ...
                  className={`p-1 rounded transition-colors ${v.enabled ? 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white' : 'text-slate-600 hover:text-slate-400'}`}
            ...
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-near-black/40 rounded border border-slate-200 dark:border-white/5 focus-within:border-slate-300 dark:focus-within:border-white/10 transition-colors">
              <input
      ...
      {envVariables.length === 0 && (
        <div className="py-8 text-center px-4">
          <div className="w-12 h-12 bg-slate-100 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto mb-3 border border-slate-200 dark:border-white/5 text-slate-600">
            <Box size={20} />
          </div>
