  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
              ...
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <div className="mt-2 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-near-black/60 px-4 py-3 font-mono text-xs text-slate-600 dark:text-slate-300">
                      {key.prefix}
                    </div>
                  </div>
                  ...
                  <span
                    className={`inline-flex self-start rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] ${
                      key.status === 'active'
                        ...
                        : 'border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.04] text-slate-500 dark:text-slate-400'
                    }`}
                  >
                    {key.status}
                  </span>
                ...
                <div className="mt-4 flex flex-col gap-4 border-t border-slate-200 dark:border-white/5 pt-4 md:flex-row md:items-center md:justify-between">
        ...
        <section className="rounded-[1.75rem] border border-slate-200 dark:border-white/10 bg-white dark:bg-[#18181b]/85 p-5">
          <div className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-400 dark:text-slate-600">
            New token
          </div>
          ...
          {isCreatingKey ? (
            <div className="mt-5 space-y-4">
              <div>
                ...
                <input
                  className="w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-near-black/70 px-4 py-3 text-sm text-slate-900 dark:text-white outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-electric-violet"
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.04] px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-slate-600 dark:text-slate-300 transition-colors hover:bg-slate-200 dark:hover:bg-white/[0.07]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 rounded-2xl bg-slate-900 dark:bg-white px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-white dark:text-near-black transition-colors hover:opacity-90 disabled:opacity-50"
                >
                  Generate
                </button>
              </div>
            ...
            <button
              onClick={() => setIsCreatingKey(true)}
              className="mt-5 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.04] px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-slate-600 dark:text-slate-300 transition-colors hover:bg-slate-200 dark:hover:bg-white/[0.07]"
            >
              Start creation
            </button>
          )}
